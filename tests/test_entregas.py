...
class FilaDuravel(unittest.TestCase):
    ...
    def test_crash_na_geracao_pede_revisao_para_nao_repetir_ferramenta(self):
        with self.store.connection:
            self.store.connection.execute("INSERT INTO entradas (id,texto,recebida_em) VALUES (1,'oi','agora')")
        chave = self.fila.iniciar_resposta([{'id':1, 'texto':'oi'}])
        self.fila.recuperar()
        self.assertIsNone(self.fila.iniciar_resposta([{'id':1, 'texto':'oi'}]))
        self.fila.resolver_entrada(1, 'reprocessar')
        self.assertEqual(self.fila.iniciar_resposta([{'id':1, 'texto':'oi'}]), chave)
